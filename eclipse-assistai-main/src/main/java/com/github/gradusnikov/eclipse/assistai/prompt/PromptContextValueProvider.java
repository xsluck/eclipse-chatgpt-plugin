...
public class PromptContextValueProvider {
    ...
    public String getContextValue(String key) {
        ...
        return switch (key) {
        case CURRENT_PROJECT_NAME -> safeGetString(
                () -> editorService.getCurrentlyOpenedFile().map(IFile::getProject).map(IProject::getName).orElse(""));
        case CURRENT_FILE_PATH -> safeGetString(() -> editorService.getCurrentlyOpenedFile()
                .map(IFile::getProjectRelativePath).map(IPath::toString).orElse(""));
        case CURRENT_FILE_NAME ->
            safeGetString(() -> editorService.getCurrentlyOpenedFile().map(IFile::getName).orElse(""));
        ...
        case ERRORS -> safeGetString(
                () -> codeAnalysisService.getCompilationErrors(getContextValue(CURRENT_PROJECT_NAME), "ERROR", -1));
