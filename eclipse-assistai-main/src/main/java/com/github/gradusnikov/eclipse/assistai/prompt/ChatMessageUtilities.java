...
public class ChatMessageUtilities {
    public static String toMarkdownContent(ChatMessage message) {
        ...
        List<ImageData> images = message.getAttachments().stream().map(Attachment::getImageData)
                .filter(Objects::nonNull).collect(Collectors.toList());

        List<String> textParts = message.getAttachments().stream().map(Attachment::toMarkdownContent)
                .filter(Objects::nonNull).collect(Collectors.toList());

        if (!images.isEmpty()) {
            content += "\n" + images.stream().map(ImageUtilities::toBase64Jpeg)
                    .map(data -> "![image](data:image/jpeg;base64," + data + ")").collect(Collectors.joining("\n"));
        }
