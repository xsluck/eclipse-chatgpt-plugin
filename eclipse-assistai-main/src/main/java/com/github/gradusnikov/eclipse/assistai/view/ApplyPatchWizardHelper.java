...
/**
 * A helper class for displaying the apply patch wizard dialog in the Eclipse
 * IDE. Allows users to apply a patch to a specified target path.
 */
@Creatable
@Singleton
public class ApplyPatchWizardHelper {
    ...
    /**
     * Displays the apply patch wizard dialog to the user, allowing them to apply a
     * patch to a specified target path.
     *
    ...
    public void showApplyPatchWizardDialog(String patch, String targetPath) {
        ...
        // TODO: for the moment assume the target is associated with the project of
    ...
    /**
     * Returns the {@link IProject} of the project associated with the currently
     * opened file in the text editor.
     *
     * @return The {@link IProject} of the project, or null if the active editor is
     *         not a text editor.
     */
    private Optional<IProject> getProjectOfCurrentlyOpenedEditor() {
        return Optional.ofNullable(PlatformUI.getWorkbench()).map(IWorkbench::getActiveWorkbenchWindow)
                .map(IWorkbenchWindow::getActivePage).map(IWorkbenchPage::getActiveEditor)
                .map(IEditorPart::getEditorInput).map(editorInput -> editorInput.getAdapter(IFile.class))
