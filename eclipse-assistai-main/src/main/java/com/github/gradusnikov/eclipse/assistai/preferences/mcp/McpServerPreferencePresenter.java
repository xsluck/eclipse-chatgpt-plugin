...
public class McpServerPreferencePresenter {
    ...
    @Inject
    public McpServerPreferencePresenter(McpClientRetistry mcpClientRetistry, ILog logger) {
    ...
    public List<McpServerDescriptorWithStatus> getServersWithStatus() {
        ...
        var list = servers.stream().map(server -> {
            try {
                ...
                var result = CompletableFuture.supplyAsync(client::ping).get(MCP_SERVER_PING_TIMEOUT_SECONDS,
                        TimeUnit.SECONDS);
    ...
    public void toggleServerEnabled(int serverIndex, boolean enabled) {
        ...
        if (serverIndex >= 0 && serverIndex < servers.size()) {
            ...
            McpServerDescriptor updated = new McpServerDescriptor(server.uid(), server.name(), server.command(),
                    server.environmentVariables(), enabled, server.builtIn());
...
//            view.showServers( getServersWithStatus() );
     ...
     * Remove a server
     * 
     * @param selectedIndex the index of the server to remove
     */
     ...
     * Save a server
     * 
     * @param selectedIndex     the index of the server to save or -1 to add a new
     *                          one
     * @param updatedServerStub the updated server data
     */
    public void saveServer(int selectedIndex, McpServerDescriptor updatedServerStub) {
        ...
        boolean nameExists = storedDescriptors.stream()
                .filter(server -> !server.uid()
                        .equals(selectedIndex >= 0 && selectedIndex < storedDescriptors.size()
                                ? storedDescriptors.get(selectedIndex).uid()
                                : ""))
        ...
        McpServerDescriptor toStore = new McpServerDescriptor(uid, updatedServerStub.name(),
                updatedServerStub.command(), updatedServerStub.environmentVariables(), updatedServerStub.enabled(),
                false);
     ...
     * 
     * @param selectedIndex the index of the server to select
     */
     ...
     * Register the view
     * 
     * @param mcpServerPreferencePage the view to register
     */
