...
public class LocalSelectionTransferHandler implements ITransferHandler {
    ...
    private void handleFile(IFile file) {
        try {
            if (isTextFile(file)) {
                var documentText = new String(Files.readAllBytes(file.getLocation().toFile().toPath()),
                        file.getCharset());
                Document document = new Document(documentText);
                presenter.onAttachmentAdded(new FileContentAttachment(file.getFullPath().toString(), 1,
                        document.getNumberOfLines(), documentText));
    ...
    private void handleCompilationUnit(ICompilationUnit compilationUnit) {
        try {
            ...
            presenter.onAttachmentAdded(new FileContentAttachment(compilationUnit.getPath().toString(), lineRange[0],
                    lineRange[1], compilationUnit.getAdapter(ISourceReference.class).getSource()));
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
        }
    }
