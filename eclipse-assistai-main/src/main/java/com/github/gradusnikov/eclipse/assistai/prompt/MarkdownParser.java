...
public class MarkdownParser {
    ...
    private static final Pattern LATEX_MULTILINE_BLOCK_OPEN_PATTERN = Pattern
            .compile("^[ \\t]*(?:\\$\\$(?!.*\\$\\$)|\\\\\\[(?!.*\\\\\\])).*$");
    private static final Pattern LATEX_SINGLELINE_BLOCK_OPEN_PATTERN = Pattern
            .compile("^[ \\t]*(?:\\$\\$(?:.*\\$\\$)|\\\\\\[(?:.*\\\\\\])).*$");
    ...
    private void handleTableSeparator(StringBuilder out, String separatorContent) {
        ...
        var formatting = Arrays.stream(separators).map(String::trim).map(separator -> switch (separator) {
    ...
    private void renderTable(StringBuilder out, MarkdownTable table) {
        ...
        if (table.hasHeader()) {
            ...
            for (int i = 0; i < headerCells.length; i++) {
                ...
                out.append("<th" + style + ">").append(convertLineToHtml(StringEscapeUtils.escapeHtml4(headerCells[i])))
                        .append("</th>\n");
            }
            out.append("</tr>\n</thead>\n");
        }
        ...
        for (int rowIndex = startRow; rowIndex < table.size(); rowIndex++) {
            ...
            for (int i = 0; i < cells.length; i++) {
                String alignment = table.getColumnFormat(i);
                String style = " style=\"text-align: " + alignment + ";\"";
                out.append("<td" + style + ">").append(convertLineToHtml(StringEscapeUtils.escapeHtml4(cells[i])))
                        .append("</td>\n");
            }
            out.append("</tr>\n");
        }
        ...
        if (!state.contains(ParserState.CODE_BLOCK)) {
            ...
            out.append(
                    """
                            ...
                            .formatted(blockClass, codeBlockId, codeBlockId, codeBlockId, lang, codeBlockId,
                                    codeBlockId, lang, codeBlockId));
     ...
     * This method wraps the LaTeX content in a {@code <span>} element with a class
     * for styling. The content is Base64 encoded to ensure that any special
     * characters are preserved and do not interfere with the HTML structure.
     *
     * @param latexBlockBuffer The buffer containing the accumulated LaTeX content.
     * @param htmlOutput       The StringBuilder to which the HTML content is
     *                         appended.
     */
    ...
    /**
     * Replaces the first occurrence of a specified pattern in the input string with
     * the given replacement.
     *
     * @param input       The original string where the replacement is to be made.
     * @param pattern     The regular expression pattern to search for in the input
     *                    string.
     * @param replacement The string to replace the first match of the pattern.
     * @return A new string with the first occurrence of the pattern replaced by the
     *         replacement string.
     */
    ...
    public static String convertMarkdownLineToHtml(String input) {
        ...
        // Task lists
        input = replaceAllPattern(input, TASK_LIST_INCOMPLETE_PATTERN,
                "<li><input type=\"checkbox\" disabled> $1</li>");
        input = replaceAllPattern(input, TASK_LIST_COMPLETE_PATTERN,
    ...
    /**
     * Converts a single line of text to HTML, processing inline elements in a
     * specific order: inline code first, then LaTeX expressions, and finally
     * Markdown formatting. This order prevents interference between different
     * syntax patterns and ensures proper escaping.
     *
     * @param line The input line containing any combination of inline code
     *             (`code`), LaTeX ($math$), and Markdown formatting
     * @return The HTML-formatted line with all inline elements converted to
     *         appropriate HTML spans with base64 encoded content
     */
    ...
    /**
     * Converts Markdown inline code segments to HTML spans with base64 encoded
     * content. Processes text enclosed in single backticks (`code`) and transforms
     * them into HTML spans with the content base64 encoded to preserve special
     * characters.
     *
     * @param line Text line potentially containing inline code segments
     * @return Line with inline code converted to HTML spans containing base64
     *         encoded content
     */
