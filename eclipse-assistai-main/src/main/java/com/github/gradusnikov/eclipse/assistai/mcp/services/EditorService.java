...
/**
 * Service interface for editor-related operations including retrieving the
...
public class EditorService {
    ...
    public Optional<IFile> getCurrentlyOpenedFile() {
        return getActiveEditor().map(IEditorPart::getEditorInput)
                .filter(editorInput -> editorInput instanceof IFileEditorInput).map(IFileEditorInput.class::cast)
    ...
    public String getCurrentlyOpenedFileContent() {
        return uiSync.syncCall(() -> {
            ...
            try {
                IFile file = getCurrentlyOpenedFile().orElseThrow(
                        () -> new RuntimeException("No active editor found or editor input not available."));
        ...
        return uiSync.syncCall(() -> {
            ...
            try {
                IEditorPart editor = getActiveEditor()
                        .orElseThrow(() -> new Exception("No active editor found. Please open a file."));
