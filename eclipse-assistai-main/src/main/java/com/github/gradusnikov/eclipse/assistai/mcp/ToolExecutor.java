...
public class ToolExecutor {
    ...
    public Method[] getFunctions() {
        return Arrays.stream(functions.getClass().getDeclaredMethods()).filter(
                method -> Objects.nonNull(method.getAnnotation(com.github.gradusnikov.eclipse.assistai.mcp.Tool.class)))
                .toArray(Method[]::new);
    }
    ...
    /**
     * Creates an array of parameter values as declared by the callback
     * {@link Method}
     * 
     * @param method
     * @param argMap
    ...
    public Object[] mapArguments(Method method, Map<String, Object> argMap) {
        return Arrays.stream(method.getParameters()).map(ToolExecutor::toParamName).map(argMap::get).toArray();
     ...
     * @return an Optional containing the function callback method, or an empty
     *         Optional if the function is not found
     */
    public Optional<Method> getFunctionCallbackByName(String name) {
        return Arrays.stream(getFunctions()).filter(method -> toFunctionName(method).equals(name)).findFirst();
    }
     ...
     * @return the parameter name, or the annotated name if present, or the default
     *         name if no annotation is found
     */
    public static String toParamName(Parameter parameter) {
        return Optional.ofNullable(parameter.getAnnotation(com.github.gradusnikov.eclipse.assistai.mcp.ToolParam.class))
                .map(ToolParam::name).filter(Predicate.not(String::isBlank)).orElse(parameter.getName());
    }
     ...
     * @return the name of the function, or the annotated name if present, or the
     *         default name if no annotation is found
     */
    public static String toFunctionName(Method method) {
        return Optional.ofNullable(method.getAnnotation(com.github.gradusnikov.eclipse.assistai.mcp.Tool.class))
                .map(Tool::name).filter(Predicate.not(String::isBlank)).orElse(method.getName());
    }
