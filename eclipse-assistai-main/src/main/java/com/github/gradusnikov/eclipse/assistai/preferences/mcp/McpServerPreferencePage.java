...
public class McpServerPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
    ...
    private void createServerDetailsForm(Composite parent) {
        ...
        nameText.setToolTipText(
                "Server name should contain only letters, numbers, underscores and hyphens ([a-zA-Z0-9_-]). Names must be unique.");
        ...
        commandText.setToolTipText(
                "Command to start the MCP server. Example: npx -y @modelcontextprotocol/server-everything dir");
    ...
    private void initializeDetailsListeners() {
        ...
        editEnvButton.addListener(SWT.Selection, e -> {
            ...
            if (selectedIndex >= 0) {
                ...
                String[] result = openEnvironmentVariableDialog("Edit Environment Variable", selectedVar.name(),
                        selectedVar.value());
                if (result != null) {
    ...
    protected void performApply() {
        ...
        McpServerDescriptor updatedServer = new McpServerDescriptor("", nameText.getText(), commandText.getText(),
                currentEnvVars, true, false);
     ...
     * 
     * @param serverDescriptor the server descriptor
     */
