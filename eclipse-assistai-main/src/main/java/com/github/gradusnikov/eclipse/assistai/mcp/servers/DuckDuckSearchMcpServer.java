...
public class DuckDuckSearchMcpServer {
    ...
    public String webSearch(@ToolParam(name = "query", description = "A search query", required = true) String query) {
        try {
            ...
            try {
                ...
                logger.error("Invalid search URL: " + url, e);
                return "Invalid search URL: " + url;
            }

            Document document = Jsoup.parse(uri.toURL(), 0);
