...
/**
 * An in-memory transport implementation that allows client and server to
 * communicate within the same JVM process. This transport is useful for testing
 * and scenarios where both client and server are running in the same
 * application.
 * 
 * <p>
 * This implementation creates a pair of transports (client and server) that are
 * linked to each other, with messages from one being delivered to the other.
 * </p>
 */
public class InMemoryTransport {
    ...
    public static TransportPair createTransportPair(ObjectMapper objectMapper) {
        ...
        InMemoryClientTransport clientTransport = new InMemoryClientTransport(serverToClientQueue, clientToServerQueue,
                clientErrorSink, objectMapper);

        InMemoryServerTransport serverTransport = new InMemoryServerTransport(clientToServerQueue, serverToClientQueue,
                serverErrorSink, objectMapper);

        return new TransportPair(clientTransport, serverTransport);
    }

    /**
     * Creates a connected pair of client and server transports with a default
     * ObjectMapper.
     * 
     * @return A pair containing client and server transports
     */
    ...
    private static class InMemoryClientTransport implements ClientMcpTransport {
        ...
        InMemoryClientTransport(BlockingQueue<McpSchema.JSONRPCMessage> inboundQueue,
                BlockingQueue<McpSchema.JSONRPCMessage> outboundQueue, Sinks.Many<String> errorSink,
...
//                            logger.error("Interrupted while waiting for messages", e);
        ...
        private void setupMessageHandler(
            ...
            this.inboundSink.asFlux().flatMap(message -> {
                // Process each message and ignore its result (convert to Mono<Void>)
                return Mono.just(message).transform(handler)
                        .contextWrite(ctx -> ctx.put("observation", "inMemoryObservation")).then(); // Convert to
                                                                                                    // Mono<Void>
            }).subscribe();
        }
        ...
        public Mono<Void> sendMessage(McpSchema.JSONRPCMessage message) {
            return Mono.fromCallable(() -> {
                ...
                try {
                    ...
                    success -> success ? Mono.empty() : Mono.error(new RuntimeException("Failed to enqueue message")))
                    .subscribeOn(Schedulers.boundedElastic()).then();
        }
    ...
    private static class InMemoryServerTransport implements ServerMcpTransport {
        ...
        InMemoryServerTransport(BlockingQueue<McpSchema.JSONRPCMessage> inboundQueue,
                BlockingQueue<McpSchema.JSONRPCMessage> outboundQueue, Sinks.Many<String> errorSink,
                ObjectMapper objectMapper) {
