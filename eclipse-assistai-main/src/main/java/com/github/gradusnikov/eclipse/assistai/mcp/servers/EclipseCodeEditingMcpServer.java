...
public class EclipseCodeEditingMcpServer {
    ...
    /**
     * LLMs have problems with correctly using replace lines, in contrary to replace
     * string.
     * <p>
     * This is their argumentation: <br>
     * <quote> When using replaceString, I only needed to identify the exact string
     * to replace and provide a new string. The tool handled the replacement
     * precisely, regardless of the internal structure of the file.
     * 
     * With replaceLines, I encountered several issues:
     * 
     * 1. Line counting error: In my first attempt, I didn't correctly account for
     * all the lines in the original code block, which led to a malformed
     * replacement where some closing braces were duplicated.
     * 
     * 2. Boundary identification: I needed to be very precise about which lines to
     * include in the replacement. With replaceString, the tool automatically finds
     * the exact string boundaries, but with replaceLines, I had to manually specify
     * start and end lines.
     * 
     * 3. Content integrity: When replacing lines, I needed to ensure that the
     * replacement content maintained the correct structure of the code, including
     * proper indentation and braces. I accidentally introduced syntax errors by not
     * properly closing the array declaration.
     * 
     * The fundamental difference is that replaceString operates on exact string
     * matching (regardless of line boundaries), while replaceLines operates on a
     * line-by-line basis that requires precise line counting and boundary
     * identification.
     * 
     ...
     * while replaceLines requires more careful handling of line numbers and content
     * structure. </quote>
     * <p>
...
//    }
    ...
    public String replaceString(
        ...
        return codeEditingService.replaceStringInFile(projectName, filePath, oldString, newString, startLineNum,
                endLineNum);
    }
